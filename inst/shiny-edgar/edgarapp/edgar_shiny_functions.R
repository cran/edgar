...
DownloadMasterShiny <- function(year.array) {
...
# Function for downloading fillings
DownloadFillingsShiny <- function(year, cik.no, form.type) {
    yr.master <- paste0(year, "master.Rda")
    stat.filling2 <- data.frame()
    
	# function to download file and return FALSE if download error
	DownlFillingShiny <- function(LINK, dest.filename) {
		tryCatch({
		  utils::download.file(LINK, dest.filename)
			return(TRUE)
		}, error = function(e) {
			return(FALSE)
		})
    ...
    if (file.exists(paste0("Master Files/", yr.master))) {
        ...
        if(nrow(year.master)>0){
			...
			if (nrow(year.master) == 0) {
				msg2 <- paste0("Form Type: ", form.type, " not filled by selected CIK")
			...
			msg3 <- paste0("Total EDGARs to be downlaoded:", total.files, "\nDo you want to downlaod the EDGAR files")
			...
			if (as.character(choice) == "yes") {
				...
				for (i in 1:total.files) {
					...
					res <- DownlFillingShiny(LINK, dest.filename)
        ...
        errmsg <- paste0("Current directory does not contains ", yr.master, 
		                 " file in 'Master Files' directory. Please download Master files using 'Get Master Index tab'.")
...
# function for downloading daily master files
GetDailyInfoShiny <- function(day, month, year) {
    date <- paste0(year, month, day)
    filename <- paste0("Daily_Index/daily_idx_", date)
    link1 <- paste0("ftp://ftp.sec.gov/edgar/daily-index/master.", date, ".idx")
    link2 <- paste0("ftp://ftp.sec.gov/edgar/daily-index/", year,
					"/QTR", ceiling(as.integer(month)/3), "/master.", date, ".idx.gz")
    link3 <- paste0("ftp://ftp.sec.gov/edgar/daily-index/", year, "/QTR",
					ceiling(as.integer(month)/3), "/master.", substr(as.character(year),
					3, 4), month, day, ".idx")
    link4 <- paste0("ftp://ftp.sec.gov/edgar/daily-index/", year, "/QTR", ceiling(as.integer(month)/3),
					"/master.", date, ".idx")
    down.success = FALSE
    	
    if (year < 1999) {
        fun.return3 <- DownlDailyMasterShiny(link3, filename)
        if (fun.return3 == 1) {
            down.success = TRUE
        }
    }
    ...
    if (year > 1998 && year < 2012) {
        fun.return4 <- DownlDailyMasterShiny(link4, filename)
        if (fun.return4 == 1) {
            down.success = TRUE
        }
    }
    ...
    if (year > 2011) {
        fun.return1 <- DownlDailyMasterShiny(link1, filename)
        if (fun.return1 == 1) {
            down.success = TRUE
        } else {
            fun.return2 <- DownlDailyMasterShiny(link2, filename)
            if (fun.return2 == 1) {
                down.success = TRUE
            }
        }
...
DownlDailyMasterShiny <- function(link, filename) {
    tryCatch({
